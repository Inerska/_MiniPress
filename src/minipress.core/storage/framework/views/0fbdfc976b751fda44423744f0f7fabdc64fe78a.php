    ...
    <div class="flex flex-col w-full p-6 space-y-4 bg-white rounded-lg shadow-md">
        <div class="flex items-center justify-between">
            ...
            <a href="/admin/articles/create"
               class="inline-block px-6 py-2 text-xs font-medium text-center text-white uppercase transition bg-blue-500 rounded-md shadow ripple hover:shadow-lg hover:bg-blue-600 focus:outline-none <?php echo e(!$authService->check() ? 'opacity-50 cursor-not-allowed' : ''); ?>"
                    <?php echo e(!$authService->check() ? 'onclick=event.preventDefault()' : ''); ?>>
                + Créer un article
            </a>
        </div>
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            if (row.published) {
                                return `<button class="unpublish-button bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" data-id="${id}">Retirer</button>`;
                            } else {
                                return `<button class="publish-button bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" data-id="${id}">Publier</button>`;
                            }
                        }
                    },
                ],
...
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/resources/views/articles/view.blade.php ENDPATH**/ ?>